...
func processToken(token string, mulRe *regexp.Regexp, enabled *bool, lineNumber int, conditionalMuls *[]MulMatch) {
...
func processCombinedMatches(line string, combinedRe *regexp.Regexp, mulRe *regexp.Regexp, lineNumber int, enabled *bool, conditionalMuls *[]MulMatch) {
	...
	for _, match := range combinedMatches {
		...
		processToken(token, mulRe, enabled, lineNumber, conditionalMuls)
	}
}

// Updates the state based on TokenType
...
func main() {
  ...
  validMulsPart1, err := parseAllValidMuls(inputFilename, mulPatternPart1)
	...
	fmt.Println("Total of all Valid Muls preceded by do():", part2Sum)
}